...
const translationService = (function () {
          ...
          this.makeRequest(sourceLanguage, targetLanguage, request)
            .then((response) => {
              ...
              for (const idx in request) {
                const result = results[idx] || { text: "", detectedLanguage: null };
                this.cbTransformResponse(result.text, dontSortResults);
    ...
    constructor() {
      super(
        ...
        function cbParseResponse(response) {
          ...
          return response.map(
            (r) => ({
              text: r.translations && r.translations[0]
                ? Utils.unescapeHTML(r.translations[0].text.replace(/<\/?b\d+>/g, ""))
                : "",
        ...
        function cbTransformResponse(result, dontSortResults) {
          ...
          } catch(e) { console.warn("DOMParser transform error:", e); }
          return resultArray.length > 0 ? resultArray : [result];
        },
        function cbGetExtraParameters(
          sourceLanguage,
          targetLanguage,
            ...
            return [
              {
                ...
                detectedLanguage: response.detectedLanguage?.language || null,
              },
            ];
          },
          function cbTransformResponse(result, dontSortResults) {
  ...
  translationService.translateHTML = async (
    ...
    if (!service) return sourceArray2d.map((arr) => arr.map(() => ""));
  ...
  translationService.translateText = async (
    ...
    const service = getSafeServiceByName(serviceName);
    if (!service) return sourceArray.map(() => "");
    return (
