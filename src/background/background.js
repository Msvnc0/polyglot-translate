...
function resetPageAction(tabId, forceShow = false) {
  if (!chrome.pageAction || !chrome.pageAction.setPopup) return;
    ...
    chrome.tabs.query({}, (tabs) =>
      tabs.forEach((tab) => {
        if (chrome.pageAction && chrome.pageAction.hide) {
          try { chrome.pageAction.hide(tab.id); } catch(e) {}
        }
      })
    );

    chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
      if (changeInfo.status == "loading" && chrome.pageAction && chrome.pageAction.hide) {
        try { chrome.pageAction.hide(tabId); } catch(e) {}
      }
    });

    try { chrome.browserAction.onClicked.addListener((tab) => {
      chrome.tabs.sendMessage(
        tab.id,
        {
        ...
        chrome.tabs.get(tabId, (tabInfo) => {
          ...
          if (chrome.pageAction && chrome.pageAction.setPopup) {
            ...
            if (twpConfig.get("showButtonInTheAddressBar") == "no") {
              if (chrome.pageAction.hide) chrome.pageAction.hide(tabId);
            } else {
              if (chrome.pageAction.show) chrome.pageAction.show(tabId);
            }
            } catch(e) {}
          }
          ...
          if (chrome.browserAction) {
            if (
              ...
              try { chrome.browserAction.setIcon({
                tabId: tabId,
...
if (typeof chrome.commands !== "undefined") {
  chrome.commands.onCommand.addListener((command) => {
    const hotkeyConfig = {
      "hotkey-toggle-translation": "hotkeyToggleTranslation",
      "hotkey-translate-selected-text": "hotkeyTranslateSelectedText",
      "hotkey-swap-page-translation-service": "hotkeySwapService",
      "hotkey-show-original": "hotkeyShowOriginal",
    };
    if (hotkeyConfig[command] && twpConfig.get(hotkeyConfig[command]) !== "yes") return;
