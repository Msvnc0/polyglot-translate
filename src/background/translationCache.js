  ...
  class Cache {
    ...
    async evictLRU(maxSize) {
      return await new Promise((resolve) => {
        ...
        transaction.onsuccess = async (event) => {
          const cursor = transaction.result;
          if (cursor) {
            ...
            entries.sort((a, b) => (a.lastAccessed || a.timestamp || 0) - (b.lastAccessed || b.timestamp || 0));
    ...
    async add(originalText, translatedText, detectedLanguage = "und") {
      ...
      const maxCacheSize = twpConfig.get("llmCacheMaxSize") || 52428800;
      if (this.translationService === "llm") {
        const currentSize = await this.getCacheSize();
        const newEntrySize = originalText.length + translatedText.length + hash.length + 200;
        if (currentSize + newEntrySize > maxCacheSize) {
          await this.evictLRU(maxCacheSize - newEntrySize);
        }
      }
